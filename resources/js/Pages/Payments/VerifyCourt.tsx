...
export default function VerifyCourt({ unpaid, paid, cancelled, filters, isAdmin }: Props) {
    ...
    const handleDateChange = (newDate: Date | undefined) => {
        ...
        setDate(newDate)
        router.get(route('payments.verify.court'), {
    ...
    const handleTabChange = (val: string) =>
        router.get(route('payments.verify.court'), { date: date ? format(date, 'yyyy-MM-dd') : '', tab: val }, { preserveState: true, replace: true })

    const submitStatus = (status: 'paid' | 'pending') => {
        if (!selectedRecord) return
        setProcessing(true)
        router.post(route('payments.verify.booking.pay', selectedRecord.id), { status }, {
            onSuccess: () => {
    ...
    const renderRow = (record: PaymentRecord, action: React.ReactNode) => (
        <tr key={record.id} className="border-b transition-colors hover:bg-muted/50">
            <td className="p-4 align-middle font-mono">{record.reference || '-'}</td>
    ...
    return (
        <AuthenticatedLayout
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Court Booking Payments</h2>}
            breadcrumbs={[
                { label: 'Dashboard', href: route('dashboard') },
                { label: 'Court Booking Payments' },
            ]}
        >
            <Head title="Court Booking Payments" />
                                ...
                                <table className="w-full text-sm">
                                    ...
                                    <tbody>
                                        {unpaid.length > 0 ? unpaid.map(r => renderRow(r,
                                            ...
                                            <tr><td colSpan={8} className="p-8 text-center text-gray-500">No unpaid court bookings found.</td></tr>
                                        )}
                                    </tbody>
                                </table>
                            </div>
                                ...
                                <table className="w-full text-sm">
                                    ...
                                    <tbody>
                                        {paid.length > 0 ? paid.map(r => renderRow(r,
                                            ...
                                            <tr><td colSpan={8} className="p-8 text-center text-gray-500">No paid court bookings found.</td></tr>
                                        )}
                                    </tbody>
                                </table>
                            </div>
                                ...
                                <table className="w-full text-sm">
                                    ...
                                    <tbody>
                                        {cancelled.length > 0 ? cancelled.map(r => renderRow(r, <span className="text-sm text-gray-400">Cancelled</span>)) : (
                                            <tr><td colSpan={8} className="p-8 text-center text-gray-500">No cancelled court bookings found.</td></tr>
                                        )}
                                    </tbody>
                                </table>
                            </div>
            ...
            <Dialog open={isMarkPaidOpen} onOpenChange={setIsMarkPaidOpen}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>Mark this court booking as <strong>PAID</strong>?</DialogDescription>
                    </DialogHeader>
                    {selectedRecord && (
                        <div className="py-4 space-y-2">
                            ...
                            <div className="flex justify-between"><span className="text-gray-500">Details:</span><span className="font-medium">{selectedRecord.details}</span></div>
            ...
            <Dialog open={isMarkPendingOpen} onOpenChange={setIsMarkPendingOpen}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>Mark this court booking as <strong>PENDING</strong>?</DialogDescription>
                    </DialogHeader>
                    {selectedRecord && (
                        <div className="py-4 space-y-2">
                            <div className="flex justify-between"><span className="text-gray-500">Customer:</span><span className="font-medium">{selectedRecord.customer}</span></div>
