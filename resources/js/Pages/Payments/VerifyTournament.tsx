...
interface Props {
    ...
    filters: { date: string; tab?: string }
    isAdmin: boolean
}

export default function VerifyTournament({ unpaid, paid, cancelled, filters, isAdmin }: Props) {
    ...
    const handleDateChange = (newDate: Date | undefined) => {
        ...
        setDate(newDate)
        router.get(route('payments.verify.tournament'), {
    ...
    const handleTabChange = (val: string) =>
        router.get(route('payments.verify.tournament'), { date: date ? format(date, 'yyyy-MM-dd') : '', tab: val }, { preserveState: true, replace: true })

    const submitStatus = (status: 'paid' | 'unpaid') => {
        if (!selectedRecord) return
        setProcessing(true)
        router.post(route('payments.verify.tournament.pay', selectedRecord.id), { status }, {
            onSuccess: () => {
                setIsMarkPaidOpen(false)
                setIsMarkPendingOpen(false)
                setSelectedRecord(null)
                setProcessing(false)
                toast.success(status === 'paid' ? "Payment confirmed." : "Payment marked as unpaid.")
            },
            onError: () => { setProcessing(false); toast.error("Failed to update payment.") },
        })
    }

    const totalUnpaid = unpaid.reduce((s, i) => s + Number(i.amount || i.expected_amount || 0), 0)
    const totalPaid = paid.reduce((s, i) => s + Number(i.amount || i.expected_amount || 0), 0)
    ...
    const renderRow = (record: PaymentRecord, action: React.ReactNode) => (
        <tr key={record.id} className="border-b transition-colors hover:bg-muted/50">
            ...
            <td className="p-4 align-middle">{record.details}</td>
            <td className="p-4 align-middle"><StatusBadge type="payment_method" value={record.method} /></td>
            <td className="p-4 align-middle font-bold">₱{Number(record.amount || record.expected_amount).toLocaleString()}</td>
    ...
    return (
        <AuthenticatedLayout
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Tournament Payments</h2>}
            breadcrumbs={[
                { label: 'Dashboard', href: route('dashboard') },
                { label: 'Tournament Payments' },
            ]}
        >
            <Head title="Tournament Payments" />
                        ...
                        <CardContent>
                            <div className="flex flex-col sm:flex-row gap-4">
                                ...
                                <div className="flex flex-shrink-0 w-full sm:w-auto items-end">
                                    <Button className="w-full sm:w-auto" onClick={() => handleDateChange(undefined)}>Show All</Button>
                                </div>
                            </div>
                                ...
                                <table className="w-full text-sm">
                                    ...
                                    <tbody>
                                        {unpaid.length > 0 ? unpaid.map(r => renderRow(r,
                                            ...
                                            <tr><td colSpan={6} className="p-8 text-center text-gray-500">No unpaid tournament registrations found.</td></tr>
                                        )}
                                    </tbody>
                                </table>
                            </div>
                                ...
                                <table className="w-full text-sm">
                                    ...
                                    <tbody>
                                        {paid.length > 0 ? paid.map(r => renderRow(r,
                                            ...
                                            <tr><td colSpan={6} className="p-8 text-center text-gray-500">No paid tournament registrations found.</td></tr>
                                        )}
                                    </tbody>
                                </table>
                            </div>
                                ...
                                <table className="w-full text-sm">
                                    ...
                                    <tbody>
                                        {cancelled.length > 0 ? cancelled.map(r => renderRow(r, <span className="text-sm text-gray-400">Cancelled</span>)) : (
                                            <tr><td colSpan={6} className="p-8 text-center text-gray-500">No cancelled tournament registrations found.</td></tr>
                                        )}
                                    </tbody>
                                </table>
                            </div>
            ...
            <Dialog open={isMarkPaidOpen} onOpenChange={setIsMarkPaidOpen}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>Mark this tournament registration as <strong>PAID</strong>?</DialogDescription>
                    </DialogHeader>
                    {selectedRecord && (
                        <div className="py-4 space-y-2">
                            ...
                            <div className="flex justify-between"><span className="text-gray-500">Tournament:</span><span className="font-medium">{selectedRecord.details}</span></div>
                            <div className="flex justify-between border-t pt-2 mt-2"><span className="text-gray-500">Amount:</span><span className="font-bold text-lg text-emerald-600">₱{Number(selectedRecord.amount || selectedRecord.expected_amount).toLocaleString()}</span></div>
                        </div>
                    )}
                    <DialogFooter>
            ...
            <Dialog open={isMarkPendingOpen} onOpenChange={setIsMarkPendingOpen}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>Mark this tournament registration as <strong>UNPAID</strong>?</DialogDescription>
                    </DialogHeader>
                    {selectedRecord && (
                        <div className="py-4 space-y-2">
                            <div className="flex justify-between"><span className="text-gray-500">Customer:</span><span className="font-medium">{selectedRecord.customer}</span></div>
                            <div className="flex justify-between"><span className="text-gray-500">Tournament:</span><span className="font-medium">{selectedRecord.details}</span></div>
                            <div className="flex justify-between border-t pt-2 mt-2"><span className="text-gray-500">Amount:</span><span className="font-bold text-lg text-red-600">₱{Number(selectedRecord.amount || selectedRecord.expected_amount).toLocaleString()}</span></div>
                        </div>
                    )}
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setIsMarkPendingOpen(false)} disabled={processing}>Cancel</Button>
                        <Button className="bg-amber-600 hover:bg-amber-700" onClick={() => submitStatus('unpaid')} disabled={processing}>
                            {processing ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <X className="w-4 h-4 mr-2" />} Mark as Unpaid
                        </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
